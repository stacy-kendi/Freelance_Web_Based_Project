    ...
    if (isset($_GET['p_id'])) {
            ...
            if (mysqli_num_rows($projectresult) > 0) {
                while($row = mysqli_fetch_assoc($projectresult)) {
                    ...
                    $projectAttachment = '../attachmentUploads/'.$row["project_attachment"];
...
<?php 
    ...
    // target the attachment file directory where the attachments will be stored when uploaded
  	$target = "../attachmentUploads/".basename($projectAttachment);

        $projectTitle = $_POST['projectTitle'];
        $projectCategory = $_POST['projectCategory'];
        $projectSkills = $_POST['requiredSkill'];
        $projectDescription = $_POST['projectDescription'];
        $projectAttachment = $_FILES['projectAttachment']['name'];// Getting File Name
        $projectAttachmentTemp = $_FILES['projectAttachment']['tmp_name'];//storing file on a temporary folder
        $similarProject = $_POST['similarProject'];
        $projectBudget = $_POST['projectBudget'];
        $projectTimeframe = $_POST['projectTimeframe'];

        move_uploaded_file($projectAttachmentTemp, "../attachmentUploads/$projectAttachment");

        //Retaining the Stored Attachment When Image is not Updated
    if(empty($projectAttachment)) {
        $query = "SELECT *FROM projects WHERE project_id = $editpostid";
        $selectAttachment = mysqli_query($connection, $query);

        while($row = mysqli_fetch_assoc($selectAttachment)) {
    ...
    if (move_uploaded_file($_FILES['projectAttachment']['tmp_name'], $target)) {
        ...
        echo "Project Updated Successfully:" . " " . "<a href='adminallprojects.php'>View All Projects</a>";
